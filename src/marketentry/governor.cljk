(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Jordan-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Jordan-resident-rep missing -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-jo-entity?
                                       true` (near-universal for Jordan
                                       ...
                                       Grounded in Jordanian legal entity
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(CCD/統一資格/GTD e-procurement登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- jo-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-jo-entity? true`, INDEPENDENTLY verify
  `:has-jo-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-jo-entity?` ground truth (most Jordan public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-jo-entity? e))
                 (not (true? (:has-jo-entity? e))))
        [{:rule :jo-entity-missing
          :detail (str subject " はヨルダン居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- ccd-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ccd? true`, INDEPENDENTLY check
  `:ccd-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ccd? e))
                 (not (true? (:ccd-verified? e))))
        [{:rule :ccd-unverified
          :detail (str subject " はCCD確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (jo-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (ccd-unverified-violations request st)
