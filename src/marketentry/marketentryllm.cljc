(ns marketentry.marketentryllm
  ...
  the JOR public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-jo-entity?=" (:has-jo-entity? e)
                        " ccd-verified?=" (:ccd-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-jo-entity? e))
                              (:has-jo-entity? e))
                          (or (not (:requires-ccd? e))
                              (:ccd-verified? e)))
                   0.9 0.3)}))
